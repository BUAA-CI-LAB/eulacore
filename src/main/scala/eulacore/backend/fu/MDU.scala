...
class Divider(len: Int = 64) extends EulaCoreModule {
  ...
  val shiftReg = RegInit(0.U((1 + len * 2).W)) //Reg(UInt((1 + len * 2).W))
  val hi = shiftReg(len * 2, len)
  val lo = shiftReg(len - 1, 0)

  val (aSign, aVal) = abs(a, io.sign)
  val (bSign, bVal) = abs(b, io.sign)
  val aSignReg = RegEnable(next = aSign, enable = newReq, init = false.B)
  val qSignReg = RegEnable(next = (aSign ^ bSign) && !divBy0, enable = newReq, init = false.B)
