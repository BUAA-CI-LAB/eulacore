...
class AXI42SimpleBusConverter() extends Module {
  ...
  // Write Path
  val aw_reg = RegInit(0.U.asTypeOf(new AXI4BundleA(idBits))) // Reg(new AXI4BundleA(idBits))
...
class SimpleBus2AXI4Converter[OT <: AXI4Lite](outType: OT, isFromCache: Boolean) extends Module {
  ...
  if (outType.getClass == classOf[AXI4]) {
    ...
    axi4.ar.bits.burst := (if (isFromCache) Mux(mem.req.bits.isBurst(), AXI4Parameters.BURST_WRAP, AXI4Parameters.BURST_INCR)
  ...
  val wen = RegEnable(next = mem.req.bits.isWrite(), enable = mem.req.fire(), init = 0.U.asTypeOf(mem.req.bits.isWrite()))
